...
// Shared horizontal breathing room so sections never stick to the screen edge
// (purge-safe inline clamp — mirrors the homepage SECTION_X_PAD).
const SECTION_X_PAD = {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1400px]" style={SECTION_X_PAD}>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-y-16 md:gap-x-20 items-center">
          {/* Questions — mobile: top, desktop: left (~40%) */}
          <div className="md:col-span-5">
            <motion.span
              ...
              className="block text-[12px] tracking-[0.34em] uppercase mb-8"
              style={{ fontFamily: 'var(--font-body)', color: 'var(--color-gold)' }}
            >
              {data.kicker}
            </motion.span>

            {/* Fade-only stagger (no y-shift) so the vertical gaps stay equal
                at every animation frame; spacing is fixed via marginBottom. */}
            <div>
              {data.questions.map((q, i) => (
                <motion.p
                  ...
                  style={{
                    ...
                    lineHeight: 1.1,
                    fontWeight: 500,
                    marginBottom: i === data.questions.length - 1 ? 0 : '40px',
                  }}
                >
                  {q}
                </motion.p>
          ...
          {/* Video — mobile: bottom, desktop: right (~58%) */}
          <motion.div
          ...
          >
            ...
            />
            <p
              className="mt-5 text-center md:text-left text-[12px] tracking-[0.28em] uppercase"
